...
- (float)countForStrategy:(uint)aStrategy fromCard:(VIPlayingCard *)startCard toCard:(VIPlayingCard *)endCard trueCount:(BOOL)isTrueCount {
    ...
    if (isTrueCount) {
        ...
        BOOL isBalanced = [[[VIPlayingCardPack infoForStrategy:aStrategy] objectForKey:@"type"] isEqualToString:@"B"];
        
        if (isBalanced) {
            // balanced strategies
            float remainingPacks = (_packCount-([startCard countForStrategy:kStrategyCount toCard:endCard startingWith:0]/(13.*4.)));
            if (remainingPacks==0) return 0;
            return theCount/remainingPacks;
        } else {
            // unbalanced strategies
            return theCount - _packCount*2.0;
        }
